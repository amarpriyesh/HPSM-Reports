...
from 
probsummarym1
where folder = 'SIEBEL-CRM' and (resolved_time+4/24>=trunc(sysdate)  and  resolved_time+4/24<=(trunc(sysdate)+20/24)) ;
...
from 
probsummarym1
where folder = 'ITSD' and (resolved_time+4/24>=trunc(sysdate)  and  resolved_time+4/24<=(trunc(sysdate)+20/24)) ;
...
echo "."| mail -s "ITSD  SIEBEL tts for date $datestr" -a SIEBEL_TT$datestr.txt.gz -a ITSD_TT$datestr.txt.gz  priyesh.a@du.ae,ankur.saxena@du.ae,gunjan.mathur@du.ae,Achin.Tiwari@du.ae,Prasanna.Kumar@du.ae
