...
B.source=C."NUMBER" and C.problem_status not in ('Closed','Resolved') and C.assignment='IT - Technical Hardware Support (Jumbo Support)'; 
spool off;
quit;
EOF
echo "."| mail -v -s "Technical Hardware Jumbo Support weekly Report for `date`" -a /hpsm/hpsm/ops/reports/THJumbo_weekly.csv THJumbo_weekly.csv Jaya.Chandran@du.ae,nihalchand.dehury@du.ae
