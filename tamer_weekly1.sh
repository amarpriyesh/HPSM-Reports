...
#quit;
#EOF
#sed -i $'s/\t/ /g' Assign_To_Others$datestr.csv
...
#quit;
#EOF
#sed -i $'s/\t/ /g' Assign_To_Ops$datestr.csv
...
#quit;
#EOF
#sed -i $'s/\t/ /g' Assign_To_Dev$datestr.csv
#sqlplus -S -R 1 HPSM94BKPADMIN/HPSM94BKPADMIN#123@sm9prod << EOF
set colsep ,
set heading on
set pagesize 0
...
#quit;
#EOF
#sed -i $'s/\t/ /g' Open_Dev$datestr.csv
...
||','||replace(replace(DBMS_LOB.SUBSTR(a.brief_description, 50,1),chr(10)),chr(13))
from probsummarym1 a where "NUMBER" not like 'PDI%' and (upper(a.assignment) in ('SD_CPE','IT - BSCS (BILLING) - DEV','IT - BSS STREAMSERVE - DEV','IT - BSS POWERBILL - DEV','IT - BSS IN - DEV','DCS - DEV','IT - BSS STRONGMAIL - DEV','IT - PAYMENT GATEWAY - DEV','IT - POS - DEV','RA - MONETA - DEV','IT - ICS - DEV','IT - SIEBEL (CRM)  ATOS','IT - OMP - DEV','IT DSP DEV','IT - ESS MNMI - DEV','IT - EAI - DEV','IT - OSS-DEV-FIXED PROVISIONING','IT - OSS-DEV-MOBILE PROVISIONING','IT - OSS-DEV-MOBILE MEDIATION','IT - GIS - DEV','IT - OSS-DEV-OAIM','IT - OSS-DEV-NPG','MSDP - DEV','IT - DEV - IDASHBOARDS','IT - OSS-DEV- HPSM','CAMPAIGN MANAGEMENT - DEV','IT - EDMS - DEV','IT - ESS DUVERSE - DEV','IT - DATAWAREHOUSE - DEV','CUSTOMER PROFITABILITY - DEV','IT - ICM DEV','IT - ERP - DEV','IT - DEV - INTRANET','IT - EBUSINESS - DEV','DQ - DEV','UCM - DEV','IT - IWV - DEV','IT - ESS MOI - DEV','IT BSS Ops L4 TT','IT ESS Ops L4 TT','IT OSS Ops L4 TT') or assignment like '%Ops L3%' or a.assignment like 'IT - Siebel (CRM)%Atos') and 
(a.resolved_time+4/24) between (TO_DATE(TO_CHAR(CURRENT_DATE, 'YYYY-MM-DD'),'YYYY-MM-DD')-7) and (TO_DATE(TO_CHAR(CURRENT_DATE, 'YYYY-MM-DD'),'YYYY-MM-DD'));
spool off;
exit;
EOF
#sed -i $'s/\t/ /g' Closed_Dev$datestr.csv
#echo -e " Dear All,\n\n PFA for the Dev TT Reports. \n\n Regards \n HPSM Team  " | mail -v -s " DEV TT Status  `date`" -a Open_Dev$datestr.csv  -a Closed_Dev$datestr.csv -a Assign_To_Others$datestr.csv -a Assign_To_Dev$datestr.csv -a Assign_To_Ops$datestr.csv  priyesh.a@du.ae,ankur.saxena@du.ae,Tamer.Awad@du.ae,LaxmaReddy.Kompally@du.ae,Shaji.Uddin@du.ae,SyedAsadAli.Shah@du.ae,Nitesh.Kumar@du.ae,Hemanta.Patra@du.ae
echo -e " Dear All,\n\n PFA for the Dev TT Reports. \n\n Regards \n HPSM Team  " | mail -v -s " DEV TT Status  `date`"  -a Open_Dev$datestr.csv  -a Closed_Dev$datestr.csv -a Assign_To_Others$datestr.csv -a Assign_To_Dev$datestr.csv -a Assign_To_Ops$datestr.csv  priyesh.a@du.ae,ankur.saxena@du.ae
