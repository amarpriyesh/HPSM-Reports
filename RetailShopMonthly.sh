...
set feedback off
set echo off
spool RetailShopMonthly$datestr.csv
-------------------Retail TT extract------------------------------------
...
B.source=C."NUMBER"(+)  and (A.contact_email like 'rs.%' or A.du_email like 'rs.%')and c.open_time>='01-Sep-2017' and A.category<>'service catalog';
spool off;
quit;
EOF
...
sed -i $'s/,/ /g' RetailShopMonthly$datestr.csv
sed -i $'s/|/,/g' RetailShopMonthly$datestr.csv
...
Best Regards,
HPSM Team"| mail -v -s "Daily Pending Retail TTs Report `date "+%d%m%y"`" -a /hpsm/hpsm/ops/reports/RetailShopMonthly$datestr.csv priyesh.a@du.ae,shihab.panakkatu@du.ae,ankur.saxena@du.ae
