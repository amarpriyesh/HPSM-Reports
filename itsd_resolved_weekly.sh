...
from probsummarym1 
where folder in('ITSD') and resolved_time+4/24 < trunc(sysdate,'D') and resolved_time+4/24 > trunc(sysdate-7,'D');
spool off;
quit;
EOF
gzip itsd_resolved_weekly$datestr.txt 
echo "Please find attached the ITSD Response Time weekly report."| mail -v -s "ITSD Response Time weekly Report" -a /hpsm/hpsm/ops/reports/itsd_resolved_weekly$datestr.csv nihalchand.dehury@du.ae,michael.milad@du.ae,jamal.sheikdawood@du.ae,mohammed.rezwanali@du.ae
