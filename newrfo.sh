...
b."NUMBER"
from activitym1 b inner join probsummarym1 a on (a."NUMBER"=b."NUMBER" ) where ( b.datestamp+4/24>=$fromdate and b.datestamp+4/24<$todate) and
b.type in ('Resolved','Rejected to Business') and is_rfo = 't' 
...
echo "To: priyesh.a@du.ae,ankur.saxena@du.ae,Gunjan.Mathur@du.ae,Neeraj.Sharma@du.ae,Sanjay.Sharma@du.ae,Bhavana.Tatti@du.ae,LaxmaReddy.Kompally@du.ae" >> $sla
echo "Subject: RFO SLA Monthly Report $fromdate-$todate" >> $sla
...
Dear All,<br /><br />Please find below RFO SLA status for $fromdate-$todate resolved/rejected to business TTs <br />
<br /></font></P></html>" >> $sla
...
echo "Content-Disposition: attachement; filename=ITSD$datestr$fromdate-$todate.csv" >> $sla
...
set echo off
set define off
spool ITSD$datestr$fromdate-$todate.csv
...
echo "" >> $sla
cat ITSD$datestr$fromdate-$todate.csv >> $sla
