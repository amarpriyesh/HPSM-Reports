...
b."NUMBER"
from activitym1 b inner join probsummarym1 a on (a."NUMBER"=b."NUMBER" ) inner join DU_BILLING_NONBILLING c on (a.CATEGORY=c.type and a.subcategory=C.AREA and a.product_type=C.sub_AREA) where ((b.datestamp+4/24)>=add_months(trunc(sysdate,'mm'),-1)  and (b.datestamp+4/24)<trunc(sysdate,'mm')) and b.type in ('Resolved','Rejected to Business') and folder ='SIEBEL-CRM'   and c.source='BILLING'
...
where  a."NUMBER" in (select    distinct
b."NUMBER"
from activitym1 b inner join probsummarym1 a on (a."NUMBER"=b."NUMBER" )  where   ((b.datestamp+4/24)>=add_months(trunc(sysdate,'mm'),-1)  and (b.datestamp+4/24)<trunc(sysdate,'mm')) and b.type in ('Resolved','Rejected to Business') and folder ='SIEBEL-CRM'   and ((CATEGORY,subcategory,product_type) NOT IN (SELECT  /*+PARALLEL(5)*/  TYPE,AREA,SUB_AREA FROM DU_BILLING_NONBILLING WHERE SOURCE='BILLING' ))
and (b.description LIKE '%IT - Billing to%' or  
b.description LIKE '%IT - CRM to%' or 
b.description LIKE '%IT - OSS-OPS-OPA to%' or
b.description LIKE '%IT - EAI to%' or
...
echo "To: priyesh.a@du.ae,ankur.saxena@du.ae" >> $sla
