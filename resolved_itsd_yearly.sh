...
select '<tr><td>'||g.assignment||'</td><td align=center>'||sum(g.jan) as jan||'</td><td align=center>'||sum(g.feb) as feb||'</td><td align=center>'||sum(g.mar) as mar||'</td><td align=center>'||sum(g.apr) as apr||'</td><td align=center>'||sum(g.may) as may||'</td><td align=center>'||sum(g.jun) as jun||'</td><td align=center>'|| sum(g.jul) as jul||'</td><td align=center>'|| sum(g.aug) as aug||'</td><td align=center>'||sum(g.sep) as sep||'</td><td align=center>'||sum(g.oct) as oct||'</td><td align=center>'||sum(g.nov) as nov||'</td><td align=center>'||sum(g.dec) as dec||'</td><td align=center>'||sum(g.tot) as tot||'</td><td align=center>'||sum(g.itsd) as itsd||'</td><td align=center>'||sum(g.fms) as fms||'</td><td align=center>'||sum(g.siebel_crm) as siebel_crm||'</td><td align=center>'||sum(g.tsrm) as tsrm||'</td><td align=center>'||sum(g.tsrm_itsd)  as tsrm_itsd ||'</td><td align=center>'||sum(g.fms_itsd)  as fms_itsd ||'</td><td align=center>'||sum(g.siebel_dealer)  as siebel_dealer||'</td></tr>' from (select * from tmp_hpsmresolved_itsd  UNION  select b.assignment,sum(b.jan) as jan,sum(b.feb) as feb,sum(b.mar) as mar,sum(b.apr) as apr,sum(b.may) as may,sum(b.jun) as jun, sum(b.jul) as jul, sum(b.aug) as aug,sum(b.sep) as sep,sum(b.oct) as oct,sum(b.nov) as nov,sum(b.dec) as dec,sum(b.tot) as tot,sum(b.itsd) as itsd,sum(b.fms) as fms,sum(b.siebel_crm) as siebel_crm,sum(b.tsrm) as tsrm,sum(b.tsrm_itsd)  as tsrm_itsd ,sum(b.fms_itsd)  as fms_itsd ,sum(b.siebel_dealer)  as siebel_dealer from  (select assignment  as assignment,0 as jan ,
0 as feb ,
0 as mar ,
0 as apr ,
0 as may ,
