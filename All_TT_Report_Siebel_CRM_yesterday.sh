...
from
probsummarym1 A
where Assignment in ('IT - Billing','IT - CRM','IT - EAI','IT - EBusiness','IT - ICS','IT - OSS-OPS-Fixed Mediation','IT - OSS-OPS-Mobile Mediation','IT - OSS-OPS-Mobile Provisioning','IT - POS','IT - Payment Gateway','NOC - IN','Bill Shock','Campaign Mgmt') AND folder='SIEBEL-CRM' and open_time+4/24 >= trunc(sysdate-1);

spool off;
quit;
EOF
gzip ALL_TT_REPORT_SIEBEL_CRM1$datestr.csv
echo "."| mail -v -s " ALL TT REPORT for SIEBEL TTs for `date -d'yesterday'`" -a /hpsm/hpsm/ops/reports/ALL_TT_REPORT_SIEBEL_CRM1$datestr.csv.gz ankur.saxena@du.ae,Rahul.Gupta1@du.ae,ankur.gahlaut@du.ae,priyesh.a@du.ae
