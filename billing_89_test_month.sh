...
case when timeToResolve(a.open_time,a.max_update) >12 and cust_val in ('Platinum')  then ericssonTTR(a.incident) else 0 end as PlatinumTTR,
case when timeToResolve(a.open_time,a.max_update) >24 and cust_val not in ('Platinum')   then ericssonTTR(a.incident) else 0 end as NONPlatinumTTR
 from
 (select
...
b."NUMBER"
from activitym1 b inner join probsummarym1 a on (a."NUMBER"=b."NUMBER" ) inner join DU_BILLING_NONBILLING c on (a.CATEGORY=c.type and a.subcategory=C.AREA and a.product_type=C.sub_AREA) where ( b.datestamp+4/24>=trunc(sysdate,'mm') and b.datestamp+4/24<trunc(sysdate)) and
...
cust_val,
timeToResolve(a.open_time,a.max_update) as ttr,
case when timeToResolve(a.open_time,a.max_update) >6 and cust_val in ('Platinum')   then ericssonTTR(a.incident) else 0 end as PlatinumTTR,
case when timeToResolve(a.open_time,a.max_update) >12 and cust_val not in ('Platinum')   then ericssonTTR(a.incident) else 0 end as NONPlatinumTTR
 from
 (select
distinct(b."NUMBER") as incident,
(a.open_time+4/24) as open_time,
...
echo "To: priyesh.a@du.ae,ankur.saxena@du.ae,Gunjan.Mathur@du.ae,Neeraj.Sharma@du.ae,Sanjay.Sharma@du.ae,Gagan.Atreya@du.ae" >> $sla
...
Dear All,<br /><br />Please find below Billing sla 8/9 status for last week (SUN-SAT) resolved/rejected to business TTs <br />
<br /></font></P></html>" >> $sla
...
(select
case when  cust_val in ('Platinum')  and (ttr<=12 or platinumttr<=12) then 1 else 0 end as platinumslab1,
case when  cust_val in ('Platinum')  and ((ttr<=24 ) or (platinumttr<=24 )) then 1 else 0 end as platinumslab2,
case when  cust_val in ('Platinum')  and (ttr>24 and  platinumttr>24) then 1 else 0 end as breachplatinum,
case when  cust_val in ('Platinum')   then 1 else 0 end as totplatinum,
case when  cust_val not in ('Platinum')  and (ttr<=24 or nonplatinumttr<=24) then 1 else 0 end as nonplatinumslab1,
case when  cust_val not in ('Platinum')  and ((ttr<=48 ) or (nonplatinumttr<=48 )) then 1 else 0 end as nonplatinumslab2,
case when  cust_val not in ('Platinum')  and (ttr>48 and nonplatinumttr>48) then 1 else 0 end as breachnonplatinum,
...
#echo "Content-Type: text/html" >> $sla
echo "<html><P><font color="blue">
Dear All,<br /><br />Please find below NON Billing sla 8/9 status for last week (SUN-SAT) resolved/rejected to business TTs <br />
<br /></font></P></html>" >> $sla
echo "<html>" >>$sla
echo "<TABLE border="1" bordercolor="BLACK">" >>$sla
echo "<TR><TH><font color="blue">slab1_plat</TH></font><TH><font color ="blue">slab2_plat</TH></font><TH><font color ="blue">breach_plat</TH></font><TH><font color ="blue">slab1_nonplat</TH></font><TH><font color ="blue">slab2_nonplat</TH></font><TH><font color ="blue">breach_nonplat</TH></font></TR>" >> $sla
...
(select round(sum(platinumslab1)*100/sum(totplatinum),2) as  slab1_plat ,round(sum(platinumslab2)*100/sum(totplatinum),2) as slab2_plat,round(sum(breachplatinum)*100/sum(totplatinum),2) as breach_plat , round(sum(nonplatinumslab1)*100/sum(totnonplatinum),2) as slab1_nonplat,round(sum(nonplatinumslab2)*100/sum(totnonplatinum),2) as slab2_nonplat,round(sum(breachnonplatinum)*100/sum(totnonplatinum),2) as breach_nonplat from
(select
case when  cust_val in ('Platinum')  and (ttr<=6 or platinumttr<=6) then 1 else 0 end as platinumslab1,
case when  cust_val in ('Platinum')  and ((ttr<=12 ) or (platinumttr<=12 )) then 1 else 0 end as platinumslab2,
case when  cust_val in ('Platinum')  and (ttr>12 and  platinumttr>12) then 1 else 0 end as breachplatinum,
case when  cust_val in ('Platinum')  then 1 else 0 end as totplatinum,
case when  cust_val not in ('Platinum')  and (ttr<=12 or nonplatinumttr<=12) then 1 else 0 end as nonplatinumslab1,
case when  cust_val not in ('Platinum') and ((ttr<=24 ) or (nonplatinumttr<=24 )) then 1 else 0 end as nonplatinumslab2,
case when  cust_val not in ('Platinum')  and (ttr>24 and nonplatinumttr>24) then 1 else 0 end as breachnonplatinum,
case when  cust_val not in ('Platinum')  then 1 else 0 end as totnonplatinum
from nonbilling89));
exit;
EOF
