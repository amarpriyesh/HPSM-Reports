...
b.datestamp+4/24 >= trunc(trunc(sysdate,'MM')-153,'MM')) and
...
echo "."| mail -v -s "Monthly IP SDP TT report for `date`" -a /hpsm/hpsm/ops/reports/Monthly_IP_SDP$datestr.csv.gz ankur.saxena@du.ae
#( echo "Please find the attached Monthly IP SDP TT report"; uuencode Monthly_IP_SDP$datestr.csv.gz Monthly_IP_SDP$datestr.csv.gz)| mailx -s "Monthly IP SDP TT report for `date`" "ankur.saxena@du.ae"
