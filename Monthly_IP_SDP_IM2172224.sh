...
set feedback off
set echo off
spool Monthly_IP_SDP_IM2172224$datestr.csv
...
b.datestamp+4/24 >= trunc(trunc(sysdate,'MM')-6,'MM')) and
...
quit;
EOF
gzip Monthly_IP_SDP_IM2172224$datestr.csv
echo "."| mail -v -s "Monthly IP SDP TT report for `date`" -a /hpsm/hpsm/ops/reports/Monthly_IP_SDP_IM2172224$datestr.csv.gz Ankur.Saxena@du.ae
#( echo "Please find the attached Monthly IP SDP TT report"; uuencode Monthly_IP_SDP_IM2172224$datestr.csv.gz Monthly_IP_SDP_IM2172224$datestr.csv.gz)| mailx -s "Monthly IP SDP TT report for `date` "Ankur.Saxena@du.ae"
