...
||','||(total -to_date('01014000000000','ddmmyyyyhh24miss'))*24*60
||','||du_assignment
...
where a.key_char=b."NUMBER" and a.name in ('akram.ibrahim','bushra.meraj','gabi.mansour','prashant.j','hussam.gebriel','auhood.alali')
and ((close_date+4/24)-(total-to_date('01014000000000','ddmmyyyyhh24miss')))< trunc(sysdate,'DD')
and ((close_date+4/24)-(total-to_date('01014000000000','ddmmyyyyhh24miss'))) >= trunc(trunc(sysdate,'DD')-7);

spool off;
quit;
EOF
echo "."| mail -v -s "Please find attached weekly UAM Incidents report" -a /hpsm/hpsm/ops/reports/UAM_Weekly$datestr.csv Hussam.Gebriel@du.ae,Akram.Ibrahim@du.ae,mohammed.rezwanali@du.ae
