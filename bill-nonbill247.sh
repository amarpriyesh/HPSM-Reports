...
a.max_update as ct,
cust_val,
timeToResolve247(a.open_time,a.max_update) as ttr,
case when timeToResolve247(a.open_time,a.max_update) >8 and cust_val in ('Platinum')  then ericssonTTR247(a.incident) else 0 end as PlatinumTTR,
case when timeToResolve247(a.open_time,a.max_update) >12 and cust_val not in ('Platinum')   then ericssonTTR247(a.incident) else 0 end as NONPlatinumTTR
 from
 (select
...
cust_val,
timeToResolve247(a.open_time,a.max_update) as ttr,
case when timeToResolve247(a.open_time,a.max_update) >6 and cust_val in ('Platinum')   then ericssonTTR247(a.incident) else 0 end as PlatinumTTR,
case when timeToResolve247(a.open_time,a.max_update) >12 and cust_val not in ('Platinum')   then ericssonTTR247(a.incident) else 0 end as NONPlatinumTTR
 from
 (select
distinct(b."NUMBER") as incident,
...
echo "To: priyesh.a@du.ae,ankur.saxena@du.ae,Gunjan.Mathur@du.ae,Neeraj.Sharma@du.ae,Sanjay.Sharma@du.ae,Shaba.Khanam@du.ae,Gagan.Atreya@du.ae" >> $sla
echo "Subject: SLA 8/9 Report version 24x7" >> $sla
...
Dear All,<br /><br />Please find below Billing sla 8/9 version 24x7 status for last week (SUN-SAT) resolved/rejected to business TTs <br />
<br /></font></P></html>" >> $sla
...
(select
case when  cust_val in ('Platinum')  and (ttr<=8 or platinumttr<=8) then 1 else 0 end as platinumslab1,
case when  cust_val in ('Platinum')  and (ttr>8 and  platinumttr>8) then 1 else 0 end as breachplatinum,
...
case when  cust_val not in ('Platinum')  and (ttr>12 and nonplatinumttr>12) then 1 else 0 end as breachnonplatinum,
...
#echo "Content-Type: text/html" >> $sla
echo "<html><P><font color="blue">
Dear All,<br /><br />Please find below NON Billing sla 8/9 version 24x7 status for last week (SUN-SAT) resolved/rejected to business TTs <br />
<br /></font></P></html>" >> $sla
echo "<html>" >>$sla
echo "<TABLE border="1" bordercolor="BLACK">" >>$sla
...
case when  cust_val in ('Platinum')  and ((ttr<=16 ) or (platinumttr<=16 )) then 1 else 0 end as platinumslab2,
case when  cust_val in ('Platinum')  and (ttr>16 and  platinumttr>16) then 1 else 0 end as breachplatinum,
case when  cust_val in ('Platinum')  then 1 else 0 end as totplatinum,
case when  cust_val not in ('Platinum')  and (ttr<=12 or nonplatinumttr<=12) then 1 else 0 end as nonplatinumslab1,
case when  cust_val not in ('Platinum') and ((ttr<=18 ) or (nonplatinumttr<=18 )) then 1 else 0 end as nonplatinumslab2,
case when  cust_val not in ('Platinum')  and (ttr>18 and nonplatinumttr>18) then 1 else 0 end as breachnonplatinum,
case when  cust_val not in ('Platinum')  then 1 else 0 end as totnonplatinum
from nonbilling89));
exit;
EOF
