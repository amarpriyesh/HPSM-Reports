...
trunc(b.datestamp+4/24)=trunc(sysdate-4) and b.type in ('Resolved','Rejected to Business') and folder ='ITSD');

spool off;
quit;
EOF
/usr/bin/smbclient \\\\172.21.15.11\\HPSMFilesShareFolder -A .passwd_new.txt -c "lcd /hpsm/hpsm/ops/reports/ ; prompt; recurse; mput historyitsd_10thJul.txt; exit;"
