...
commit;
delete from pending_alert3 where "NUMBER" in ( select a."NUMBER" from pending_alert3 a, probsummarym1 b where a."NUMBER"=b."NUMBER"  and b.problem_status in ('Resolved','Closed','Rejected','Rejected to Business'))  ;
commit;
...
||','||emaillead
||','||description
from  ( select * from  pending_alert3  order by Pending_Date desc );
spool off
...
echo -e "Dear Gunjan,\n\n Please find pending user data in attachment.\n\nRegards\nHPSM Team" | mail -s "Pending user data" -a escalation3.csv -a pending_alert.csv gunjan.mathur@du.ae,priyesh.a@du.ae
