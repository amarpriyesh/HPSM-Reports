...
||'|'||du_sla_end
||'|'||replace(replace(DBMS_LOB.SUBSTR(brief_description, 2000,1),chr(10)),chr(13))
||'|'||replace(replace(DBMS_LOB.SUBSTR(action, 2000,1),chr(10)),chr(13))
...
quit;
EOF
/usr/bin/smbclient \\\\172.21.15.11\\HPSMFilesShareFolder -A .passwd_new.txt -c "lcd /hpsm/hpsm/ops/reports/ ; prompt; recurse; mput ITNOC_daily$datestr.txt; exit;"
echo "."| mail -v -s "IT NOC TT details for `date`" -a /hpsm/hpsm/ops/reports/ITNOC_daily$datestr.txt IT.DCO@du.ae,utkarsh.jauhari@du.ae,ITNOC@du.ae
