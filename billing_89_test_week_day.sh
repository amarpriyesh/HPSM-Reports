...
b."NUMBER"
from activitym1 b inner join probsummarym1 a on (a."NUMBER"=b."NUMBER" ) inner join DU_BILLING_NONBILLING c on (a.CATEGORY=c.type and a.subcategory=C.AREA and a.product_type=C.sub_AREA) where ( b.datestamp+4/24>=trunc(sysdate)-4 and b.datestamp+4/24<trunc(sysdate)) and
